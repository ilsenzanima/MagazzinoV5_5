-- =====================================================
-- FIX: Filtro Basse Scorte (Esclude Esauriti)
-- =====================================================
-- Modifica la logica "low_stock" per escludere articoli con quantità <= 0
-- (che devono apparire solo in "out_of_stock").
-- =====================================================
...
BEGIN
  ...
  WITH fi AS (
    ...
    AND (p_status IS NULL OR p_status = 'all' OR
         ...
         (p_status = 'low_stock' AND i.quantity <= i.min_stock AND i.quantity > 0)) -- FIX: Added AND quantity > 0
  )
