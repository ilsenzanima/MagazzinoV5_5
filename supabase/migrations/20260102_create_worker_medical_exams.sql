...
CREATE TABLE IF NOT EXISTS public.worker_medical_exams (
    ...
    next_exam_date date NOT NULL, -- Typically 12 months after exam_date
    doctor_name text,
    notes text,
...
CREATE INDEX IF NOT EXISTS idx_worker_medical_exams_worker_id ON public.worker_medical_exams(worker_id);

-- Enable RLS
ALTER TABLE public.worker_medical_exams ENABLE ROW LEVEL SECURITY;

-- RLS Policies (drop first if exists, then create)
DROP POLICY IF EXISTS "Allow authenticated read" ON public.worker_medical_exams;
DROP POLICY IF EXISTS "Allow authenticated insert" ON public.worker_medical_exams;
DROP POLICY IF EXISTS "Allow authenticated update" ON public.worker_medical_exams;
DROP POLICY IF EXISTS "Allow authenticated delete" ON public.worker_medical_exams;
