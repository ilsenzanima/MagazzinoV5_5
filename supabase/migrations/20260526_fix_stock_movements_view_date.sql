-- Migration: Fix date field in stock_movements_view
-- Date: 2026-05-26
-- Description: Replace dni.created_at with dn.date for delivery_note_items rows.
--              The correct semantic date for a DDT line is the date set on the
--              delivery note header (dn.date), not when the DB row was created.
--              This also fixes the "Costi e SAL" page showing wrong dates after
--              items are moved between bolle (moved items kept their old created_at).
...
SELECT
    dni.id,
    dn.date::timestamptz as date,   -- FIX: usa la data della bolla (dn.date) invece di dni.created_at
    dn.type,
    CASE
