-- Il totale di una bolla mostrato nella pagina "Movimentazioni" (MovementDetailContent)
-- usa come prezzo unitario `i.price || i.purchase_items?.price || i.inventory?.price || 0`
-- (src/lib/services/delivery-notes.ts mapDbToDeliveryNote): in JS l'operatore || salta
-- anche un prezzo salvato come 0, non solo NULL. La vista stock_movements_view, usata
-- invece dalla pagina Costi delle commesse (src/lib/services/movements.ts -> JobCostiSAL),
-- saltava del tutto delivery_note_items.price e usava solo
-- COALESCE(purchase_items.price, inventory.price), quindi con un prezzo di riga a 0 o
-- diverso da quello dell'acquisto collegato le due pagine mostravano due totali diversi
-- per la stessa bolla. NULLIF(...,0) replica la stessa semantica "0 = non impostato".
...
UNION ALL
 SELECT dni.id,
    ...
    i.model AS item_model,
    i.unit AS item_unit,
    COALESCE(NULLIF(dni.price, 0), NULLIF(pi.price, 0), i.price) AS item_price,
    dni.pieces,
    dni.coefficient,
    dn.notes,
    dn.job_id,
