...
export function ProposalSiteDocuments({ proposalId }: Props) {
    ...
    const load = async () => {
        try {
            ...
            const [docs, types, proposalFolders] = await Promise.all([
                ...
                jobSiteDocumentFoldersApi.getByProposalId(proposalId),
            ])
            setDocuments(docs)
            setDocTypes(types)
    ...
    // restano nella cartella piatta "Documenti Cantiere" (comportamento storico).
    const typeBaseSegments = (type?: JobSiteDocumentType) => {
        const base = ['Proposte', proposalId, 'Documenti Cantiere']
    ...
    const handleMove = async () => {
        ...
        try {
            ...
            await proposalSiteDocumentsApi.update(movingDoc.id, { folderId: targetFolderId })
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            if (editFile) {
                ...
                const ext = compressed.name.split('.').pop() || ''
                const uploaded = await uploadFileToDrive(compressed, ['Proposte', proposalId, 'Documenti Cantiere'])
                fileUrl = uploaded.fileId
                fileType = ext
                fileSize = compressed.size
            }
